...
public class ProductManagementController {
    ...
    @FXML private VBox paneCreate;
    @FXML private VBox paneEdit;

    // Form Tạo mới
    @FXML private TextField txtCreateName;
    ...
    @FXML private TextArea txtEditDesc;
    @FXML private VBox uploadBox;
    @FXML private ComboBox<String> cbAuctionDuration;
    ...
    private HBox createProductCardUI(Product product) {
        ...
        try {
            if (product.getImagePath() != null && !product.getImagePath().equals("null") && !product.getImagePath().isEmpty()) {
        ...
        imgContainer.setStyle("-fx-background-color: #334155; -fx-background-radius: 8;");
        ...
        lblStatusTag.getStyleClass().add("status-badge");

        // Logic phân loại Tag dựa trên trạng thái
        switch (product.getStatus()) {
            case "WAITING":
                lblStatusTag.setText("Chờ duyệt");
                lblStatusTag.setStyle("-fx-background-color: #F59E0B; -fx-text-fill: white; -fx-background-radius: 12; -fx-padding: 4 12;"); // Màu cam
                break;
            ...
            default:
                ...
                lblStatusTag.getStyleClass().add("badge-waiting");
        }
    ...
    private void resetUploadUI() {
        ...
        iconLabel.setStyle("-fx-font-size: 30px; -fx-text-fill: #3B82F6;");

        Label mainText = new Label("Tải ảnh sản phẩm");
        mainText.setStyle("-fx-text-fill: #F8FAFC; -fx-font-weight: bold;");

        Label subText = new Label("PNG, JPG tối đa 5MB");
        subText.setStyle("-fx-text-fill: #94A3B8; -fx-font-size: 11px;");
    ...
    void handleDeleteProduct(ActionEvent event) {
        ...
        String status = currentEditingProduct.getStatus();
        if (status.equals("LIVE") || status.equals("SOLD")) {
            showAlert("Thông báo", "Sản phẩm đang đấu giá hoặc đã kết thúc, không thể xóa!", Alert.AlertType.WARNING);
            return;
        }
        ...
        confirmAlert.setContentText("Hành động này không thể hoàn tác: " + currentEditingProduct.getName());
    ...
    void handlePublishAuction(ActionEvent event) {
        ...
        if (status.equals("WAITING")) {
            showAlert("Thông báo", "Sản phẩm đang chờ Admin duyệt, chưa thể đăng lên sàn!", Alert.AlertType.WARNING);
            return;
        ...
        confirmAlert.setContentText("Sản phẩm sẽ hiển thị trên Khu vực chính với thời gian là " + durationStr + ". Bạn có chắc chắn?");
    ...
    private int parseDurationToMinutes(String durationStr) {
        switch (durationStr) {
            ...
            case "12 giờ": return 720;
            case "24 giờ": return 1440;
            default: return 60;
        }
    }
    ...
    class Product {
        ...
        public Product(String id, String name, String price, String desc, String status, String imagePath) {
            this.id = id;
            this.name = name;
            this.price = price;
