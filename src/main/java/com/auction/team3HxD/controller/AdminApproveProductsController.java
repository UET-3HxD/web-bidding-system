...
public class AdminApproveProductsController {
  ...
  @FXML
  private Label lblSidebarName, lblSidebarAvatar, lblPendingCount;
  @FXML
  ...
  private HBox createPendingCard(String[] data) {
    ...
    imageBox.getStyleClass().add("pending-image-box");
    ...
    if (imageBox.getChildren().isEmpty()) {
      ...
      noImg.getStyleClass().add("no-image-icon");
    ...
    lblName.getStyleClass().add("pending-product-name");

    Label lblSeller = new Label("Người bán: " + seller);
    lblSeller.getStyleClass().add("pending-seller-name");

    Label lblPrice = new Label(price.replace(".0", "") + " đ");
    lblPrice.getStyleClass().add("pending-price");

    Label lblDesc = new Label(desc);
    lblDesc.getStyleClass().add("pending-desc");
