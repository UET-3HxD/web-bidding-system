...
public class MyBidsController {
    ...
    @FXML
    private Label lblDetailYourBid;
    @FXML
    ...
    private void loadBidList(String message) {
        ...
        for (int i = 1; i < parts.length; i++) {
            ...
            if (record.isEmpty() || record.equals("EMPTY")) {
                continue;
            }
    ...
    private HBox createBidCard(String[] data) {
        ...
        imageBox.getStyleClass().add("bid-image-box");
        try {
        ...
        if (imageBox.getChildren().isEmpty()) {
            ...
            noImg.getStyleClass().add("bid-no-image");
        ...
        switch (status) {
            case "ACTIVE":
                ...
                lblStatus.getStyleClass().setAll("status-badge", "status-active");
                break;
            case "WON":
                lblStatus.setText("Đã thắng");
                lblStatus.getStyleClass().setAll("status-badge", "status-won");
                break;
            case "LOST":
                lblStatus.setText("Đã thua");
                lblStatus.getStyleClass().setAll("status-badge", "status-lost");
                break;
            default:
                lblStatus.setText(status);
                lblStatus.getStyleClass().setAll("status-badge", "status-default");
        }
    ...
    private void showBidDetail(String[] data) {
        ...
        // Badge trạng thái chi tiết
        switch (status) {
            case "ACTIVE":
                lblDetailStatus.setText("Đang diễn ra");
                lblDetailStatus.getStyleClass().setAll("detail-status-active");
                btnEnterRoom.setVisible(true);
                break;
            case "WON":
                lblDetailStatus.setText("Đã thắng");
                lblDetailStatus.getStyleClass().setAll("detail-status-won");
                btnEnterRoom.setVisible(false);
                break;
            case "LOST":
                lblDetailStatus.setText("Đã thua");
                lblDetailStatus.getStyleClass().setAll("detail-status-lost");
                btnEnterRoom.setVisible(false);
                break;
            default:
    ...
    void handleEnterBidRoom(ActionEvent event) {
        ...
        if ("ACTIVE".equals(selectedStatus)) {
            ...
            SceneSwitcher.getInstance()
                    .switchTo("/fxml/bid_room.fxml", (Node) event.getSource(), "Phòng đấu giá");
        } else {
